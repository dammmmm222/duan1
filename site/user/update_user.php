...
if (isset($_SESSION['id_user']) && ($_SESSION['id_user'] > 0)) {
    $tai_khoan = lay_tai_khoan_theo_ma($_SESSION['id_user']);
}
if (is_array($tai_khoan) && ($tai_khoan !=[])) {
    extract($tai_khoan);
...
<body style="font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif" class="mx-auto max-w-screen-2xl">
    ...
    <div class="MENU border-[1px] border-gray-200 grid grid-cols-3">
        ...
        <form class="right bg-[#F6F5F3] col-span-2"  method="post" action="../index.php?act=cap_nhat_tk">
                <input type="hidden" name="id" value="<?= $id ?>">
           
        <div class="mx-32 my-20">
        <a  href ="sign_in.php">
</a>
                <h1 class="font-bold text-3xl ">CẬP NHẬT TÀI KHOẢN</h1>
                <hr class="mt-5 mb-10">
                <h3 class="font-bold text-2xl ">CẬP NHẬTTHÔNG TIN</h3>
                <div class="bg-[#FFFFFF] my-10 rounded">
                    <div class="mx-10 py-10 space-y-4">
                        <label for="">Email *</label><br>
                        <input type="email"  value="<?= $email ?>" placeholder="name@example.com" name="email" class="py-4 px-4 w-full border-2 rounded border-[#CFCCC8]" required>
                    <div class="mx-10 py-10 space-y-4">
                        <label for="">Tên tài khoản *</label><br>
                        <input type="text"  value="<?= $user_name ?>" placeholder="Username1234 ?" name="user_name" class="py-4 px-4 w-full border-2 rounded border-[#CFCCC8]" required>
                    </div>
                    <div class="mx-10 py-10 space-y-4">
                        <label for="">Mật khẩu *</label><br>
                        <input type="password"  value="<?= $password ?>" placeholder="********" name="password" class="py-4 px-4 w-full border-2 rounded border-[#CFCCC8]" required>
                    </div>
                    <div class="mx-10 py-10 space-y-4">
                        <label for="">Họ và tên *</label><br>
                        <input type="text"  value="<?= $full_name ?>" placeholder="Nguyễn Văn A" name="full_name" class="py-4 px-4 w-full border-2 rounded border-[#CFCCC8]" required>
                    </div>
                    <div class="mx-10 py-10 space-y-4">
                        <label for="">Địa chỉ nhà *</label><br>
                        <input type="text"  value="<?= $address ?>" placeholder="Trịnh Văn Bô , Hà Nội Street" name="address" class="py-4 px-4 w-full border-2 rounded border-[#CFCCC8]" required>
                    </div>
                    <div class="mx-10 py-10 space-y-4">
                        <label for="">Điện thoại *</label><br>
                        <input type="text"  value="<?= $phone ?>" placeholder="098+ ?? " name="phone" class="py-4 px-4 w-full border-2 rounded border-[#CFCCC8]" required>
                    </div>
                </div>
                <input class="font-semibold text-xl px-6 py-2 bg-black text-[#FFFFFF]" value="Xác nhận" type="submit" name="btn_update">
            </div>
        </form>
        </div>
