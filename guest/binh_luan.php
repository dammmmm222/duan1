...
function lay_binh_luan_theo_hh($ma_hang_hoa)
{
  ...
  $sql = "SELECT bl.*, hh.product_name, kh.id FROM product_rev bl JOIN product hh ON bl.id_product = hh.id JOIN user kh ON kh.id = bl.id_user WHERE bl.id_product = $ma_hang_hoa ORDER BY ngay_bl DESC";
  $ds_binh_luan = pdo_query($sql);
  return $ds_binh_luan;
}
